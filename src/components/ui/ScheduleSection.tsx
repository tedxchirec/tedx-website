...
const schedule: ScheduleItem[] = [
  ...
  },
  {
    ...
    description: "An immersive audio-visual experience to ignite your curiosity.",
    type: "talk",
  },
  {
    time: "10:30",
  ...
  return (
    <section className="relative py-20 md:py-32 px-4 md:px-0 min-h-[80vh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none -z-10">
        <div className="absolute left-1/4 top-10 w-40 h-40 bg-red-500/10 rounded-full blur-2xl animate-float" style={{animationDelay:'0.5s'}}></div>
        <div className="absolute right-10 top-1/3 w-24 h-24 bg-white/10 rounded-full blur-xl animate-pulse" style={{animationDelay:'1.2s'}}></div>
        <div className="absolute left-8 bottom-10 w-28 h-28 bg-red-400/10 rounded-full blur-2xl animate-float" style={{animationDelay:'2s'}}></div>
        <div className="absolute right-1/4 bottom-0 w-32 h-32 bg-red-500/10 rounded-full blur-2xl animate-float" style={{animationDelay:'2.5s'}}></div>
      </div>

      <h2 className="text-3xl md:text-5xl font-bold text-center mb-2">Event Schedule</h2>
      <div className="text-lg md:text-xl text-gray-300 text-center mb-12">October 26, 2024 • Virtual & Brussels</div>
          ...
          {schedule.map((item, idx) => {
            ...
            return (
              <React.Fragment key={item.time + item.title}>
                {/* Left card */}
                <div className={`flex justify-end items-center min-h-[160px] ${isLeft ? '' : 'invisible'}`}>
                  {isLeft && (
                    <div className="w-full max-w-md bg-[#151922] border border-white/5 rounded-2xl shadow-lg px-6 py-5 md:py-6 flex flex-col gap-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-lg md:text-xl font-bold text-red-500 mr-2 min-w-[60px]">{item.time}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${typeColors[item.type] || 'bg-gray-700/70 text-gray-200'}`}>{item.type}</span>
                      </div>
                      <div className="text-lg md:text-2xl font-bold text-white leading-tight">{item.title}</div>
                      {item.subtitle && <div className="text-sm text-gray-400 font-medium mb-1">{item.subtitle}</div>}
                      <div className="text-sm md:text-base text-gray-300">{item.description}</div>
                    </div>
                  )}
                </div>
                {/* Center timeline/dot */}
                <div className="flex flex-col items-center relative min-h-[160px]">
                  ...
                  {idx < schedule.length - 1 && (
                    <span className="absolute left-1/2 -translate-x-1/2 top-1/2 w-1 h-[160px] bg-gradient-to-b from-red-900/30 via-white/10 to-red-900/30 z-0" style={{ marginTop: '32px' }} />
                  )}
                </div>
                {/* Right card */}
                <div className={`flex justify-start items-center min-h-[160px] ${!isLeft ? '' : 'invisible'}`}>
                  {!isLeft && (
                    <div className="w-full max-w-md bg-[#151922] border border-white/5 rounded-2xl shadow-lg px-6 py-5 md:py-6 flex flex-col gap-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-lg md:text-xl font-bold text-red-500 mr-2 min-w-[60px]">{item.time}</span>
          ...
          {schedule.map((item) => (
            <div key={item.time + item.title} className="flex items-center">
              ...
              <div className="w-full max-w-md mx-auto bg-[#151922] border border-white/5 rounded-2xl shadow-lg px-6 py-5 flex flex-col gap-2">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-lg font-bold text-red-500 mr-2 min-w-[60px]">{item.time}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${typeColors[item.type] || 'bg-gray-700/70 text-gray-200'}`}>{item.type}</span>
                </div>
                <div className="text-lg font-bold text-white leading-tight">{item.title}</div>
                {item.subtitle && <div className="text-sm text-gray-400 font-medium mb-1">{item.subtitle}</div>}
